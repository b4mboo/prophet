...
describe Prophet do
  let(:request_id) { 42 }
  let(:request) { PullRequest.new @api_response }
  ...
  it 'loops through all open pull requests' do
    ...
    PullRequest.should_receive(:new).with(@api_response)
  ...
  describe '?switch_branch_to_merged_state' do
    subject { prophet.send :switch_branch_to_merged_state }
